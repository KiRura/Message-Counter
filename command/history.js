...
export default {
	data: new SlashCommandBuilder()
		...
		.addIntegerOption((option) =>
			option.setName("page").setDescription("ページ数").setMinValue(1),
		),
	/**
	...
	async execute(interaction) {
		...
		if (guild.history.length === 0)
			return await interaction.reply("データがひとつもありませんでした。");
		const maxpages = Math.floor(guild.history.length / 10) + 1;
		if (maxpages < page)
			return await interaction.reply({
				content: `${maxpages}ページまで指定できます`,
				ephemeral: true,
			});
		...
		const history = guild.history
			...
			.map((data) => {
				return `${functions.dateToString(new Date(Number(data.timestamp)))}: ${
					data.count
				}`;
			});
