...
client.on(Events.InteractionCreate, async interaction => {
  ...
  try {
    ...
    await interaction.user
      ...
      .catch(_error => {});
    await (
      await interaction.client.users.fetch("606093171151208448")
    ).send(`誰かがinteractionエラーを吐いた\n${error}`);
  }
});
...
client.on(Events.MessageCreate, async message => {
  ...
  try {
    ...
    console.error(error);
    await (
      await message.client.users.fetch("606093171151208448")
    ).send(`誰かがmessageCreateエラーを吐いた\n${error}`);
  }
});
